...
defult_url = query_params['url'][0] if 'url' in query_params else \
    'https://app.pollination.cloud/chriswmackey/projects/demo/jobs/0cd8f29b-71e1-44be-9ce2-7d4c6e4e5d13/runs/ec6bbd7e-1579-550c-9e89-2ba424cd2d04'
...
if run is not None:
    ...
    with info_c:
        with st.expander('See model breakdown'):
            ...
            st.write(df.columns)
            format_dict = {key: '{:.2f}' for key in df.columns if key != 'Space Name'}
            st.table(df.style.format(formatter=format_dict))
